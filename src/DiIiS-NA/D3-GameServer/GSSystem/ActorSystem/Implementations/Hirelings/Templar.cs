...
namespace DiIiS_NA.GameServer.GSSystem.ActorSystem.Implementations.Hirelings
{
	...
	public class Templar : Hireling
	{
		...
		{
			var dbhireling = player.World.Game.GameDbSession.SessionQueryWhere<DBHireling>(dbh => dbh.DBToon.Id == player.Toon.PersistentID && dbh.Class == 1).ToList().FirstOrDefault();
