...
namespace DiIiS_NA.GameServer.GSSystem.ActorSystem.Implementations.Hirelings
{
	...
	public class Scoundrel : Hireling
	{
		...
		{
			var dbhireling = player.World.Game.GameDbSession.SessionQueryWhere<DBHireling>(dbh => dbh.DBToon.Id == player.Toon.PersistentID && dbh.Class == 2).ToList().FirstOrDefault();
